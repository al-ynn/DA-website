...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'save', payload: Partial<Record<MainCategory, CategorySelection>>): void
}>()
...
const TASK_DEFINITIONS: Record<MainCategory, TaskItem[]> = {
  ...
  chemist: [
    ...
    'Available Phosphorus',
    {
      ...
      displayName: 'Exchangeable Bases (Potassium, Calcium, Magnesium, Sodium)',
...
function getDefaultActiveCategory() {
  if (props.initialCategory && props.availableCategories.includes(props.initialCategory)) {
...
function getTaskIdByName(category: MainCategory, taskName: string) {
  return props.taskCategories[category]?.laboratory_tasks.find((task) => task.name === taskName)?.id
}
...
function buildSelections() {
  ...
  ;(['admin', 'chemist', 'agriculturist'] as MainCategory[]).forEach((category) => {
...
watch(
  () => [props.availableCategories, props.initialCategory, props.initialSelections],
  () => {
...
const activeTasks = computed(() => TASK_DEFINITIONS[activeCategory.value] ?? [])

const hasPresetSection = computed(
  () => activeCategory.value !== 'admin' && activePresets.value.length > 0,
)
...
function togglePreset(presetId: number) {
...
function selectAllTasks() {
  selections.value[activeCategory.value].taskIds = getFlatTaskIds(activeCategory.value)
}
...
function selectAllPresets() {
  selections.value[activeCategory.value].presetIds = activePresets.value.map((preset) => preset.id)
}
...
<template>
  ...
  >
    <div class="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl dark:border-zinc-800 dark:bg-[#111111]">
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5">
        <div class="flex flex-col gap-6">
          ...
          >
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                "
              >
                <component :is="getCategoryIcon(roleCategory)" class="h-4 w-4" />
                {{ getCategoryLabel(roleCategory) }}
              </button>
            </div>
          </div>

          <template v-if="hasPresetSection">
              ...
              <div class="grid gap-3">
                ...
                >
                  <input
                    ...
                    :checked="selections[activeCategory].presetIds.includes(preset.id)"
            ...
            <div class="relative">
              ...
              <div class="relative flex justify-center">
                <span class="bg-white px-3 text-sm font-medium text-zinc-500 dark:bg-[#111111] dark:text-zinc-400">
                  OR SELECT MANUALLY
                </span>
              </div>
            </div>
          ...
          <div class="flex flex-col gap-3">
            ...
            <div class="flex flex-col gap-3">
              <template v-for="(item, idx) in activeTasks" :key="idx">
                <template v-if="typeof item === 'string'">
                  <label class="flex cursor-pointer items-center gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3 transition hover:bg-zinc-50 dark:border-zinc-700 dark:bg-[#111111] dark:hover:bg-[#171717]">
                    <input
                      type="checkbox"
                      class="h-5 w-5 rounded border-zinc-300 text-black focus:ring-0 dark:border-zinc-600 dark:bg-[#0b0b0b]"
                      :checked="selections[activeCategory].taskIds.includes(getTaskIdByName(activeCategory, item) ?? -1)"
                      @change="getTaskIdByName(activeCategory, item) !== undefined && toggleTask(getTaskIdByName(activeCategory, item)!)"
                    />
                ...
                <template v-else>
                  <div class="rounded-2xl border border-zinc-200 bg-white px-4 py-4 dark:border-zinc-700 dark:bg-[#111111]">
                    <div class="border-b border-zinc-200 pb-3 text-sm font-semibold text-zinc-900 dark:border-zinc-800 dark:text-white">
                      {{ item.category }}
                    </div>
                    ...
                    <div class="mt-3 grid gap-3 sm:grid-cols-2">
                      ...
                      >
                        <input
                          type="checkbox"
                          class="h-5 w-5 rounded border-zinc-300 text-black focus:ring-0 dark:border-zinc-600 dark:bg-[#0b0b0b]"
                          :checked="selections[activeCategory].taskIds.includes(getTaskIdByName(activeCategory, subtask) ?? -1)"
                          @change="getTaskIdByName(activeCategory, subtask) !== undefined && toggleTask(getTaskIdByName(activeCategory, subtask)!)"
                        />
      ...
      <div class="flex items-center justify-end gap-3 border-t border-zinc-200 px-6 py-5 dark:border-zinc-800">
        <button
          type="button"
          @click="emit('close')"
