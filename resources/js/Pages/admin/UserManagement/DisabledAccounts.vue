...
type AccountStatus = 'ACTIVE' | 'DISABLED' | 'DRAFT'
...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Disabled Accounts', href: '/user-management/disabled' },
]
...
const disabledUsers = computed(() =>
  users.value.filter((user) => user.status === 'DISABLED'),
)
...
const filteredUsers = computed(() => {
  const filtered = disabledUsers.value.filter((user) => {
...
function enableAccount(id: number) {
    router.patch(`/user-management/${id}/enable`)
}
...
function viewAccount(user: UserRecord) {
  router.visit(`/user-management/${user.id}?source=disabled`)
}
