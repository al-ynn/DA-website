...
const ENDPOINTS = {
    userTaskPreset: (userId: number) =>
        `/task-management/users/${userId}/task-presets`,
    ...
    presetClear: '/task-management/presets/clear',
} as const;
...
const selectedRoleCategory = computed(() => {
    ...
    return category
        ? (taskCategoryMap.value[category]?.title ?? category)
        : null;
});

function formatFullName(user: UserRecord) {
    return [user.surname, user.givenname, user.middlename]
...
function getCommonEditableCategories(users: UserRecord[]) {
    ...
    return ['admin', 'chemist', 'agriculturist'].filter((category) =>
        users.every((user) =>
            getEditableCategoriesForUser(user).includes(
...
function isSelectableInCategory(user: UserRecord, category: MainCategory) {
    ...
    return (
        ...
        user.additionalCategories.includes(category)
    );
}
...
function getUsersWithTasks(category: MainCategory) {
    return getVisibleUsers(category).filter(
        (user) => getDisplayedTaskGroups(user, category).length > 0,
    );
}
...
function getSelectedUsers(category: MainCategory) {
    return props.users.filter(
        (user) =>
            ...
            isSelectableInCategory(user, category),
    );
}
...
function clearTasksForSelected(category: MainCategory, users: UserRecord[]) {
    const userIds = users
        .filter((user) => selectedUsers[category].has(user.id))
        .map((user) => user.id);
    ...
    const clearSelection = () => {
        const next = new Set(selectedUsers[category]);
        userIds.forEach((userId) => next.delete(userId));
        selectedUsers[category] = next;
    };
    ...
    router.put(
        ...
        { user_ids: userIds, task_presets: [], task_ids: [] },
        {
            preserveScroll: true,
            onSuccess: clearSelection,
        },
...
function getCommonPresetIdsForCategory(
    users: UserRecord[],
    category: MainCategory,
) {
    if (!users.length) return [];
    ...
    const common = new Set(
        users[0].assigned_presets
            .filter(
                (preset) =>
                    category === 'admin' || preset.category === category,
            )
...
function getCommonTaskIds(users: UserRecord[], category: MainCategory) {
    ...
    const categoryTasksByUser = users.map((user) =>
        user.assigned_tasks.filter(
            (task) => category === 'admin' || task.category === category,
        ),
    );
...
function buildInitialSelections(
    ...
    const selections: Partial<
        Record<MainCategory, { presetIds: number[]; taskIds: number[] }>
    > = {};
    ...
    categories.forEach((category) => {
        ...
        if (users.length === 1) {
            selections[category] = getCurrentSelectionsForUser(
                users[0],
                category,
            );
            return;
...
function getPresetTaskIds(user: UserRecord, category: MainCategory) {
    return user.assigned_presets
        .filter(
            (preset) => category === 'admin' || preset.category === category,
        )
...
function getDisplayedTaskGroups(user: UserRecord, category: MainCategory) {
    ...
    const individualTasks = user.assigned_tasks
        .filter(
            (task) =>
                (category === 'admin' || task.category === category) &&
                !presetTaskIds.has(task.id),
        )
        .map((task) => ({
            type: 'task' as const,
            id: task.id,
...
function getDisplayedPresetGroups(user: UserRecord, category: MainCategory) {
    return getDisplayedTaskGroups(user, category).filter(
        (group) => group.type === 'preset',
    );
}

function getDisplayedIndividualTasks(user: UserRecord, category: MainCategory) {
    return getDisplayedTaskGroups(user, category).filter(
...
function openAssignmentModal(
    ...
    assignmentInitialSelections.value = buildInitialSelections(
        users,
        assignmentAvailableCategories.value,
    );

    const currentSelection = assignmentInitialSelections.value[category] ?? {
        presetIds: [],
        taskIds: [],
    };
    assignmentInitialSelections.value[category] = {
        presetIds: [...currentSelection.presetIds],
        taskIds: Array.from(
            new Set([...currentSelection.taskIds, ...initialTaskIds]),
        ),
    };
...
function removePresetFromUser(user: UserRecord, presetId: number) {
    ...
    router.put(
        ENDPOINTS.userTaskPreset(user.id),
        {
            ...
            task_ids: user.assigned_tasks.map((task) => task.id),
        },
        {
            preserveScroll: true,
        },
...
function handlePresetClear(category: 'chemist' | 'agriculturist') {
    router.post(
        ENDPOINTS.presetClear,
        { category },
        {
            preserveScroll: true,
        },
...
function handlePresetUse(payload: {
    category: 'chemist' | 'agriculturist';
    taskIds: number[];
}) {
    const users = getSelectedUsers(payload.category);

    if (!users.length) {
        selectionWarningMessage.value = `Select at least one ${payload.category === 'chemist' ? 'Chemist' : 'Agriculturist'} account before using a preset.`;
        showSelectionWarning.value = true;
        return;
    }

    closeManagePresets();
    openAssignmentModal(payload.category, users, payload.taskIds);
}
</script>
    ...
    <AppSidebarLayout :breadcrumbs="breadcrumbs">
        <div class="relative p-2 sm:p-3 md:p-4 lg:p-5">
            <div class="flex flex-col gap-3 sm:gap-4">
                <div
                    class="sticky top-0 z-30 -mx-2 -mt-2 border-b border-border bg-background px-2 pb-3 pt-2 sm:-mx-3 sm:-mt-3 sm:px-3 md:-mx-4 md:-mt-4 md:px-4 md:pt-4"
                >
                    <div class="flex flex-col gap-3">
                        ...
                        >
                            <div>
                                ...
                                <p class="mt-1 text-xs text-muted-foreground">
                                    Assign and manage tasks across different
                        ...
                        <div
                            class="rounded-xl border border-border bg-card p-2.5 text-card-foreground shadow-sm sm:p-3"
                        >
                            <div class="grid gap-2 md:grid-cols-3">
                                <div class="relative md:col-span-2">
                                    ...
                                    <input
                                        ...
                                        class="h-9 w-full rounded-lg border border-input bg-background pl-10 pr-3 text-xs text-foreground outline-none ring-0 placeholder:text-muted-foreground focus:border-ring"
                                    />
                                </div>

                                <select
                                    v-model="selectedFilter"
                                    class="h-9 rounded-lg border border-input bg-background px-3 text-xs text-foreground outline-none focus:border-ring"
                                >
                                    <option value="all">
                                        All Roles (Main Task)
                                    </option>
                        ...
                        >
                            <div class="border-b border-zinc-200 px-3 py-2.5">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0E3D1A] text-white"
                                    >
                                        <component
                                    ...
                                    <div>
                                        <h2
                                            class="text-sm font-semibold tracking-wide text-zinc-900 sm:text-base"
                                        >
                                            {{
                                                getCategoryLabel(
                                                    category,
                            ...
                            <div class="p-2.5 sm:p-3">
                                <div class="flex flex-col gap-3">
                                    <template
                                        v-if="
                                            getUsersWithTasks(category).length >
                                            0
                                        "
                                    >
                                        <div class="flex flex-col gap-2.5">
                                            ...
                                            <div
                                                class="no-scrollbar overflow-x-auto rounded-xl border border-zinc-200"
                                            >
                                                <table
                                                    class="w-full min-w-[680px] table-fixed text-[12px] sm:min-w-[760px] sm:text-[13px] lg:min-w-full"
                                                >
                                                    <colgroup>
                                                        <col class="w-[40px]" />
                                                        <col class="w-[30%]" />
                                                    ...
                                                    <thead>
                                                        <tr class="bg-zinc-50">
                                                            <th
                                                                class="px-2 py-2 text-left sm:px-3 sm:py-2.5"
                                                            ></th>
                                                            <th
                                                                class="px-2 py-2 text-left text-xs font-medium text-zinc-700 sm:px-3 sm:py-2.5"
                                                            >
                                                                Name
                                                            </th>
                                                            <th
                                                            ...
                                                            <th
                                                                class="px-2 py-2 text-center text-xs font-medium text-zinc-700 sm:px-3 sm:py-2.5"
                                                            >
                                                                Action
                                                            </th>
                                                        </tr>
                                                    ...
                                                    <tbody>
                                                        <tr
                                                            v-for="user in getUsersWithTasks(
                                                                category,
                                                            )"
                                                            :key="user.id"
                                                        ...
                                                        >
                                                            <td
                                                                class="px-2 py-2 align-top sm:px-3 sm:py-2.5"
                                                            >
                                                                <button
                                                                    type="button"
                                                                    :disabled="
                                                                        !isSelectableInCategory(
                                                                            user,
                                                                            category,
                                                                        )
                                                                    "
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    ...
                                                                    >
                                                                        <div
                                                                            class="break-words text-[12px] font-medium leading-5 text-zinc-900 sm:text-[13px]"
                                                                        >
                                                                            {{
                                                                                formatFullName(
                                                                                    user,
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    ...
                                                                    >
                                                                        <div
                                                                            v-for="preset in getDisplayedPresetGroups(
                                                                                user,
                                                                                category,
                                                                            )"
                                                                            :key="`${user.id}-preset-${preset.id}`"
                                                                            class="inline-flex max-w-full flex-col items-start gap-1 rounded-lg border border-zinc-200 bg-zinc-50 px-2 py-1 text-[10px] font-semibold text-zinc-900"
                                                                        >
                                                                            <div
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <button
                                                                                    ...
                                                                                    "
                                                                                    class="shrink-0 rounded p-0.5 text-zinc-500 transition hover:bg-rose-50 hover:text-rose-600"
                                                                                >
                                                                                    <X
                                                                                        class="h-3 w-3"
                                                                                    />
                                                                            ...
                                                                            >
                                                                                <span
                                                                                    ...
                                                                                    :key="`${user.id}-preset-${preset.id}-task-${task}`"
                                                                                    class="inline-flex rounded-full bg-[#020826] px-2 py-0.5 text-[9px] font-semibold text-white"
                                                                                >
                                                                                    {{
                                                                                        task
                                                                                    }}
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        :class="
                                                                            getDisplayedPresetGroups(
                                                                                ...
                                                                                .length >
                                                                            0
                                                                                ? 'border-t border-zinc-200 pt-1.5'
                                                                                : ''
                                                                        "
                                                                    >
                                                                        <span
                                                                            v-for="task in getDisplayedIndividualTasks(
                                                                                user,
                                                                                category,
                                                                            )"
                                                                            :key="`${user.id}-task-${task.id}`"
                                                            ...
                                                            <td
                                                                class="px-2 py-2 text-center align-top sm:px-3 sm:py-2.5"
                                                            >
                                                                <button
                                                                    type="button"
                                                                    @click="
                                                                        openAssignmentForUser(
                                                                            category,
                                                                            user,
                                                                        )
                                                                    "
                                        ...
                                        <div class="flex flex-col gap-2.5">
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <tbody>
                                                        <tr
                                                            v-for="user in getUsersWithoutTasks(
                                                                category,
                                                            )"
                                                            :key="user.id"
                                                            class="border-t border-zinc-200"
                                                        ...
                                                        >
                                                            ...
                                                            <td
                                                                class="px-2 py-2 align-top text-zinc-400 sm:px-3 sm:py-2.5"
                                                            >
                                                                -
                                                            </td>
                                    ...
                                    >
                                        <div
                                            class="rounded-xl border border-dashed border-zinc-200 px-4 py-6 text-sm text-zinc-500"
                                        >
        ...
        >
            <div
                class="w-full max-w-md rounded-xl border border-zinc-200 bg-white shadow-xl"
            >
                <div
                    class="flex items-start gap-3 border-b border-zinc-200 px-5 py-4"
                >
                    <div
                        class="mt-0.5 flex h-9 w-9 items-center justify-center rounded-full bg-amber-100 text-amber-600"
                    >
                        <Check class="h-4 w-4" />
                    </div>
                ...
                <div
                    class="flex justify-end border-t border-zinc-200 px-5 py-4"
                >
                    <button
                        type="button"
